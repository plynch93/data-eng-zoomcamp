...
apt-get install -y apt-transport-https ca-certificates curl software-properties-common | tee -a $LOG_FILE
curl -fsSL https://download.docker.com/linux/ubuntu/gpg | apt-key add - | tee -a $LOG_FILE
add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable" | tee -a $LOG_FILE
apt-get update | tee -a $LOG_FILE
apt-get install -y docker-ce | tee -a $LOG_FILE

# Add user to the docker group
echo "Adding user to the docker group..." | tee -a $LOG_FILE
usermod -aG docker ${USER} | tee -a $LOG_FILE

# Enable and start Docker service
echo "Enabling and starting Docker service..." | tee -a $LOG_FILE
...
sudo mount -o discard,defaults /dev/disk/by-id/google-${MACHINE_NAME}-disk /mnt/disks/additional-disk
...
echo "/dev/disk/by-id/google-${MACHINE_NAME}-disk /mnt/disks/additional-disk ext4 discard,defaults,nofail 0 2" | sudo tee -a /etc/fstab
